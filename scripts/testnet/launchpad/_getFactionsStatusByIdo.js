...
async function main() {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let factionId = IDO_ADDR !== ethers.constants.AddressZero ? data[i].faction.id : data[i].id
...
async function getSubgraphDataByIdo() {
    let query = '{ factionByIDOs (where:{ido:"' + IDO_ADDR.toLowerCase() + '"}) { id prefunded energy nPrefunders ido { id } faction { id } } }'
    return await querySubgraph(query)
}
