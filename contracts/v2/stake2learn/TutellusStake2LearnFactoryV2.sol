...
contract TutellusStake2LearnFactoryV2 is EIP712Upgradeable, BeaconFactory, UUPSUpgradeableByRole {
    ...
    address public constant NEW_POOL = 0xB4aea291c29ED30968FBE648e7Bb0C2e7D6Dbaa1;
    ...
    function _canUpgradeByImplementation(address _implementation, address sender) internal view returns (bool) {
        (_implementation);
    ...
    function upgradeByImplementation(address _implementation) public {
        require(_canUpgradeByImplementation(_implementation, msg.sender), "TUTS2L001");
        _upgradeByImplementation(_implementation);
    }
    ...
    function _getPairDecimals() internal view returns (uint256) {
        IUniswapV2Pair _pool = IUniswapV2Pair(NEW_POOL);
        address token0 = _pool.token0();
        address pairAddress = token0 == NEW_TOKEN ? _pool.token1() : token0;
    ...
    function _getPoolReserves() internal view returns (uint256 reservesToken, uint256 reservesPair) {
        IGT3Pair _pool = IGT3Pair(NEW_POOL);
        (uint256 reserves0, uint256 reserves1,) = _pool.getReserves();
        (reservesToken, reservesPair) = _pool.token0() == NEW_TOKEN ? (reserves0, reserves1) : (reserves1, reserves0);
    }
