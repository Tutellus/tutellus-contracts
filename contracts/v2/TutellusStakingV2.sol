...
import "../interfaces/ITutellusRewardsDistributor.sol";
...
contract TutellusStakingV2 is UUPSUpgradeableByRole {
    ...
    function _update() internal {
      ITutellusRewardsDistributor rewardsInterface = ITutellusRewardsDistributor(ITutellusManager(config).get(keccak256("REWARDS")));
      uint256 released = rewardsInterface.released(ITutellusManager(config).name(address(this))) - _released;
      _released += released;
      if(balance > 0) {
    ...
    function setFees(uint256 minFee_, uint256 maxFee_) public onlyRole(DEFAULT_ADMIN_ROLE) {
      ...
      require(minFee_ <= 1e20, "TutellusStaking: minFee cannot exceed 100 ether");
      require(maxFee_ <= 1e20, "TutellusStaking: maxFee cannot exceed 100 ether");
      minFee = minFee_;
      maxFee = maxFee_;
    ...
    function emergencyWithdraw() whenPaused public returns (uint256) {
      uint256 amount = getUserBalance(msg.sender);
      require(amount > 0, "TutellusStaking: no tokens to withdraw");
      ITutellusERC20 tokenInterface = ITutellusERC20(ITutellusManager(config).get(keccak256("ERC20")));
